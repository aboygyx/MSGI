...
class AttentionLayerWin(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        queries = queries.view(B* max(1, L//self.window_size), self.window_size, H, -1)
        keys = keys.view(B* max(1, S//self.window_size), self.window_size, H, -1)
        values = values.view(B* max(1, S//self.window_size), self.window_size, H, -1)
